...
export type FileCluster = {
  id: string
  label: string
  category: SimulatedFileItem['category']
...
export type AlignmentValidation = {
  id: string
  source: 'human-centered-alignment-layer'
  checks: Array<{
    id: string
    label: string
    status: 'passed' | 'watch'
...
export type TimeRecoveryEstimate = {
...
export type OrganizationProposal = {
  ...
  checklist: string[]
  timeRecoveryEstimate: TimeRecoveryEstimate
  clarityEstimate: {
    beforeScore: number
    afterScore: number
    label: string
...
export type OrganizationSimulation = {
  ...
  scenario: string
  governanceMode: 'proposal-first'
  datasetSource: 'mock-only'
  ...
  genesisReflections: ReflectionObservation[]
...
export const HUMANITY_GUIDE_ORGANIZATION_SIMULATION: OrganizationSimulation = {
  ...
  chaosSignals: [
    ...
    },
    {
      ...
      label: 'Naming drift',
      severity: 'medium',
      description: 'File names use mixed languages, repeated final markers, timestamps, and informal labels.'
    }
  ],
  genioAnalysis: {
    summary:
  ...
  proposal: {
    ...
    groups: [
      ...
      },
      {
        ...
        itemCount: 1,
        priority: 'low',
        rationale: 'Temporary exports need owner review before any future deletion flow.'
      }
    ],
    duplicateGroups: [
      {
