...
describe('LabPage', () => {
  ...
  it('renders locked initially and unlocks with local input state', async () => {
    ...
    expect(screen.getByText('Recruiter demo')).toBeInTheDocument()
    expect(
      screen.getByText('Humanity Guide OS turns simulated clutter into a governed organization proposal.')
    ).toBeInTheDocument()
    expect(screen.getByText('Elevator pitch')).toBeInTheDocument()
    expect(screen.getByText('Problem -> Analysis -> Proposal -> Human Approval')).toBeInTheDocument()
    expect(screen.getByText('What this demo proves')).toBeInTheDocument()
    expect(screen.getByText('Human workspaces drift into clutter')).toBeInTheDocument()
    expect(screen.getByText('GENIO converts chaos into a governed plan')).toBeInTheDocument()
    expect(screen.getByText('The output is reviewable and reversible')).toBeInTheDocument()
    expect(screen.getByText('The Owner remains in control')).toBeInTheDocument()
    expect(screen.getByText('Why this matters')).toBeInTheDocument()
    expect(screen.getByText('Professional value in plain language')).toBeInTheDocument()
    ...
    expect(screen.getAllByText('Simulation complete').length).toBeGreaterThan(0)
    ...
    expect(screen.getByText('GENIO analyzes priorities and risk')).toBeInTheDocument()
    ...
    expect(screen.getByText('GENESIS observes contextual overload')).toBeInTheDocument()
    expect(screen.getByLabelText('GENESIS Reflection Feed')).toBeInTheDocument()
    expect(screen.getAllByText('Alignment Validation Card').length).toBeGreaterThan(0)
    expect(screen.getByText('Alignment protects human limits')).toBeInTheDocument()
    ...
    expect(screen.getByText('Email Organization Preview Panel')).toBeInTheDocument()
    expect(screen.getByText('Controlled Email Organization Preview')).toBeInTheDocument()
    expect(screen.getByText('Inbox Chaos Summary')).toBeInTheDocument()
    expect(screen.getByText('GENIO Email Analysis Card')).toBeInTheDocument()
    expect(screen.getByText('GENESIS Communication Pattern Reflection')).toBeInTheDocument()
    expect(screen.getAllByText('Alignment Validation Card').length).toBeGreaterThan(0)
    expect(screen.getByText('Suggested Labels / Categories')).toBeInTheDocument()
    expect(screen.getByText('Priority Inbox Proposal')).toBeInTheDocument()
    expect(screen.getByText('Draft Suggestions Preview')).toBeInTheDocument()
    expect(screen.getByText('Human Approval Required')).toBeInTheDocument()
    expect(screen.getAllByText('simulationOnly: true').length).toBeGreaterThan(0)
    expect(screen.getByText('executionMode: email-preview-only')).toBeInTheDocument()
    expect(screen.getByText('emailSendAccess: false')).toBeInTheDocument()
    expect(screen.getByText('emailDeleteAccess: false')).toBeInTheDocument()
    expect(screen.getByText('emailMoveAccess: false')).toBeInTheDocument()
    expect(screen.getByText('emailReplyAccess: false')).toBeInTheDocument()
    expect(screen.getAllByText('emailDraftMode: preview-only').length).toBeGreaterThan(0)
    expect(screen.getByText('Controlled Execution Planning')).toBeInTheDocument()
    expect(screen.getByText('Controlled Execution Planning Layer')).toBeInTheDocument()
    expect(screen.getAllByText('Execution blocked by default').length).toBeGreaterThan(0)
    expect(screen.getAllByText('No real execution performed').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Manual execution only').length).toBeGreaterThan(0)
    expect(screen.getByText('Execution timeline preview')).toBeInTheDocument()
    expect(screen.getAllByText('Analysis').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Risk Evaluation').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Human Approval').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Simulation Preview').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Rollback Prepared').length).toBeGreaterThan(0)
    expect(screen.getByText('Manual Execution Recommended')).toBeInTheDocument()
    expect(screen.getAllByText('Rollback Ready').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Impact Scope').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Human Verification Required').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Execution Boundaries').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Safety Status').length).toBeGreaterThan(0)
    expect(screen.getByText('Governance approval chain preview')).toBeInTheDocument()
    expect(screen.getByText('GENIO Governance')).toBeInTheDocument()
    expect(screen.getByText('Alignment Layer')).toBeInTheDocument()
    expect(screen.getByText('Safety Validation')).toBeInTheDocument()
    expect(screen.getByText('Simulation Approval')).toBeInTheDocument()
    expect(screen.getByText('Manual Human Execution')).toBeInTheDocument()
