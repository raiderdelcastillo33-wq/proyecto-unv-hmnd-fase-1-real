...
export async function runAiTask(rawInput: AiRunInput): Promise<AiRunResult> {
  ...
  if (!apiKey) {
    const fallbackOutput =
      input.task === 'summary'
        ? `Fallback summary: ${input.text.slice(0, 180)}${input.text.length > 180 ? '...' : ''}`
        : input.task === 'translation'
          ? `[fallback-${input.targetLanguage ?? 'en'}] ${input.text}`
          : `1) Validate OPENAI_API_KEY in your deployment.\n2) Retry this request when API credentials are configured.\n3) Keep a local fallback for demos to avoid blocking UX.`
